<template>
  <section>
    <div id="vsvhome" class="no-js">
      <div class="vsv-home-group vsv-home-group_dest-she vsv-home-group_5">
        <div data-bannerid="9545" class="vsv-bannercontainer">
          <div
            id="vsv-newnow"
            data-id="9545"
            data-cta="/tr/kadin/one-cikanlar/yeni_d18713172"
            data-islinkable="/tr/kadin/one-cikanlar/yeni_d18713172"
            class="vsv-box"
          >
            <div class="vsv-blocs">
              <div
                class="vsv-bloc"
                style="background-color: rgba(255, 255, 255, 0.4)"
              >
                <div
                  data-device="desktop,tabletlandscape,tablet"
                  data-width="100"
                  data-vsvbg="//staticpages.mngbcn.com/homes/images/fw22/she/noviembre/she_newnow_0711_mobile.jpg"
                  data-vsv-bg-processed="//staticpages.mngbcn.com/homes/images/fw22/she/noviembre/she_newnow_0711_mobile.jpg?imwidth=1292&amp;imdensity=1"
                  class="vsv-image"
                  style="
                    background-image: url('//staticpages.mngbcn.com/homes/images/fw22/she/noviembre/she_newnow_0711_mobile.jpg?imwidth=1292&amp;imdensity=1');
                    transition: opacity 0.5s ease 0s;
                    opacity: 1;
                  "
                ></div>
                <div class="vsv-overlay"></div>
                <div class="vsv-bloc-wrapper">
                  <div class="vsv-titles">
                    <h2 class="vsv-title">Yeni</h2>
                    <div class="vsv-links">
                      <a
                        href="/tr/kadin/one-cikanlar/yeni_d18713172"
                        title="Go to 	Yeni"
                        class="vsv-link"
                        ><span class="hvr-underline-fix2"
                          >Daha fazla keşfet</span
                        ></a
                      >
                    </div>
                  </div>
                </div>
              </div>
            </div>
          </div>
        </div>
        <div data-bannerid="9495" class="vsv-bannercontainer">
          <div
            id="vsv-giftguide-she"
            data-id="9495"
            data-cta="/tr/kadin/one-cikanlar/hediye-rehberi_d20689879"
            data-islinkable="/tr/kadin/one-cikanlar/hediye-rehberi_d20689879"
            class="vsv-box"
          >
            <div class="vsv-blocs">
              <div
                class="vsv-bloc"
                style="background-color: rgba(69, 64, 58, 0.4)"
              >
                <div
                  data-device="desktop,tabletlandscape,tablet"
                  data-akaset=""
                  data-vsvbg="//staticpages.mngbcn.com/homes/images/fw22/she/noviembre/she_landing_gg_0711.jpg"
                  data-vsv-bg-processed="//staticpages.mngbcn.com/homes/images/fw22/she/noviembre/she_landing_gg_0711.jpg?imwidth=429&amp;imdensity=1&amp;impolicy=set_13"
                  class="vsv-image data-vsvbg-50"
                  style="
                    background-image: url('//staticpages.mngbcn.com/homes/images/fw22/she/noviembre/she_landing_gg_0711.jpg?imwidth=429&amp;imdensity=1&amp;impolicy=set_13');
                    transition: opacity 0.5s ease 0s;
                    opacity: 1;
                  "
                ></div>
                <div class="vsv-overlay"></div>
                <div class="vsv-bloc-wrapper">
                  <div class="vsv-titles">
                    <h2 class="vsv-title">Hediye Rehberi</h2>
                    <div class="vsv-links">
                      <a
                        href="/tr/kadin/one-cikanlar/hediye-rehberi_d20689879"
                        title="Go to Hediye Rehberi "
                        class="vsv-link"
                        ><span class="hvr-underline-fix2"
                          >Daha fazla keşfet</span
                        ></a
                      >
                    </div>
                  </div>
                </div>
              </div>
            </div>
          </div>
        </div>
        <div data-bannerid="9539" class="vsv-bannercontainer">
          <div
            id="vsv-fiestaeventos"
            data-id="9539"
            data-cta="/tr/kadin/one-cikanlar/parti-ve-ozel-gunler_d12911309"
            data-islinkable="/tr/kadin/one-cikanlar/parti-ve-ozel-gunler_d12911309"
            class="vsv-box"
          >
            <div class="vsv-blocs">
              <div
                class="vsv-bloc"
                style="background-color: rgba(98, 94, 94, 0.4)"
              >
                <div
                  class="vsv-image data-vsvbg-50"
                  data-device="desktop,tabletlandscape,tablet"
                  data-akaset=""
                  data-vsvbg="//staticpages.mngbcn.com/homes/images/fw22/she/noviembre/she_landing_fiesta_0711.jpg"
                  data-vsv-bg-processed="//staticpages.mngbcn.com/homes/images/fw22/she/noviembre/she_landing_fiesta_0711.jpg?imwidth=429&amp;imdensity=1&amp;impolicy=set_13"
                  style="
                    background-image: url('//staticpages.mngbcn.com/homes/images/fw22/she/noviembre/she_landing_fiesta_0711.jpg?imwidth=429&amp;imdensity=1&amp;impolicy=set_13');
                    transition: opacity 0.5s ease 0s;
                    opacity: 1;
                  "
                ></div>
                <div class="vsv-overlay"></div>
                <div class="vsv-bloc-wrapper">
                  <div class="vsv-titles">
                    <h2 class="vsv-title">Parti ve özel günler</h2>
                    <div class="vsv-links">
                      <a
                        href="/tr/kadin/one-cikanlar/parti-ve-ozel-gunler_d12911309"
                        title="Go to Parti ve özel günler"
                        class="vsv-link"
                        ><span class="hvr-underline-fix2"
                          >Daha fazla keşfet</span
                        ></a
                      >
                    </div>
                  </div>
                </div>
              </div>
            </div>
            <div class="vsv-blocs">
              <div
                class="vsv-bloc"
                style="background-color: rgba(98, 94, 94, 0.4)"
              >
                <div
                  class="vsv-image data-vsvbg-50"
                  data-device="desktop,tabletlandscape,tablet"
                  data-akaset=""
                  data-vsvbg="//staticpages.mngbcn.com/homes/images/fw22/she/noviembre/she_landing_fiesta_0711.jpg"
                  data-vsv-bg-processed="//staticpages.mngbcn.com/homes/images/fw22/she/noviembre/she_landing_fiesta_0711.jpg?imwidth=429&amp;imdensity=1&amp;impolicy=set_13"
                  style="
                    background-image: url('//staticpages.mngbcn.com/homes/images/fw22/she/diciembre/she_landing_puffers_0512.jpg?imwidth=433&imdensity=1&impolicy=set_13');
                    transition: opacity 0.5s ease 0s;
                    opacity: 1;
                  "
                ></div>
                <div class="vsv-overlay"></div>
                <div class="vsv-bloc-wrapper">
                  <div class="vsv-titles">
                    <h2 class="vsv-title">Buffer Bar</h2>
                    <div class="vsv-links">
                      <a
                        href="/tr/kadin/one-cikanlar/parti-ve-ozel-gunler_d12911309"
                        title="Go to Parti ve özel günler"
                        class="vsv-link"
                        ><span class="hvr-underline-fix2"
                          >Daha fazla keşfet</span
                        ></a
                      >
                    </div>
                  </div>
                </div>
              </div>
            </div>
          </div>
        </div>

        <div data-bannerid="9540" class="vsv-bannercontainer">
          <div
            id="vsv-exclusivoonline"
            data-id="9540"
            data-cta="/tr/kadin/one-cikanlar/online-exclusive_d19236523"
            data-islinkable="/tr/kadin/one-cikanlar/online-exclusive_d19236523"
            class="vsv-box"
          >
            <div class="vsv-blocs">
              <div
                class="vsv-bloc"
                style="background-color: rgba(96, 96, 96, 0.4)"
              >
                <div
                  class="vsv-image data-vsvbg-50"
                  data-device="desktop,tabletlandscape,tablet"
                  data-akaset=""
                  data-vsvbg="//staticpages.mngbcn.com/homes/images/fw22/she/noviembre/she_landing_exonline_0711.jpg"
                  data-vsv-bg-processed="//staticpages.mngbcn.com/homes/images/fw22/she/noviembre/she_landing_exonline_0711.jpg?imwidth=429&amp;imdensity=1&amp;impolicy=set_13"
                  style="
                    background-image: url('//staticpages.mngbcn.com/homes/images/fw22/she/diciembre/she_landing_leather_0512.jpg?imwidth=433&imdensity=1&impolicy=set_13');
                    transition: opacity 0.5s ease 0s;
                    opacity: 1;
                  "
                ></div>
                <div class="vsv-overlay"></div>
                <div class="vsv-bloc-wrapper">
                  <div class="vsv-titles">
                    <h2 class="vsv-title">Leather and More</h2>
                    <div class="vsv-links">
                      <a
                        href="/tr/kadin/one-cikanlar/online-exclusive_d19236523"
                        title="Go to Online Exclusive"
                        class="vsv-link"
                        ><span class="hvr-underline-fix2"
                          >Daha fazla keşfet</span
                        ></a
                      >
                    </div>
                  </div>
                </div>
              </div>
            </div>
            <div class="vsv-blocs">
              <div
                class="vsv-bloc"
                style="background-color: rgba(96, 96, 96, 0.4)"
              >
                <div
                  class="vsv-image data-vsvbg-50"
                  data-device="desktop,tabletlandscape,tablet"
                  data-akaset=""
                  data-vsvbg="//staticpages.mngbcn.com/homes/images/fw22/she/noviembre/she_landing_exonline_0711.jpg"
                  data-vsv-bg-processed="//staticpages.mngbcn.com/homes/images/fw22/she/noviembre/she_landing_exonline_0711.jpg?imwidth=429&amp;imdensity=1&amp;impolicy=set_13"
                  style="
                    background-image: url('//staticpages.mngbcn.com/homes/images/fw22/she/noviembre/she_landing_exonline_0711.jpg?imwidth=429&amp;imdensity=1&amp;impolicy=set_13');
                    transition: opacity 0.5s ease 0s;
                    opacity: 1;
                  "
                ></div>
                <div class="vsv-overlay"></div>
                <div class="vsv-bloc-wrapper">
                  <div class="vsv-titles">
                    <h2 class="vsv-title">Online Exclusive</h2>
                    <div class="vsv-links">
                      <a
                        href="/tr/kadin/one-cikanlar/online-exclusive_d19236523"
                        title="Go to Online Exclusive"
                        class="vsv-link"
                        ><span class="hvr-underline-fix2"
                          >Daha fazla keşfet</span
                        ></a
                      >
                    </div>
                  </div>
                </div>
              </div>
            </div>
          </div>
        </div>
        <div class="vsv-bannercontainer" data-bannerid="9541">
          <div
            class="vsv-box"
            id="vsv-keytrends"
            data-id="9541"
            data-cta="/tr/kadin/one-cikanlar/ayın-trendleri_d21332319"
            data-islinkable="/tr/kadin/one-cikanlar/ayın-trendleri_d21332319"
          >
            <div class="vsv-blocs">
              <div class="vsv-bloc" style="background-color: #14131166">
                <div
                  class="vsv-image data-vsvbg-50"
                  data-device="desktop,tabletlandscape,tablet"
                  data-akaset=""
                  data-vsvbg="//staticpages.mngbcn.com/homes/images/fw22/she/noviembre/she_landing_keytrends_0711.jpg"
                  data-vsv-bg-processed="//staticpages.mngbcn.com/homes/images/fw22/she/noviembre/she_landing_keytrends_0711.jpg?imwidth=429&amp;imdensity=1&amp;impolicy=set_13"
                  style="
                    background-image: url('//staticpages.mngbcn.com/homes/images/fw22/she/diciembre/she_landing_exonline_0512.jpg?imwidth=433&imdensity=1&impolicy=set_13');
                    transition: opacity 0.5s ease 0s;
                    opacity: 1;
                  "
                ></div>

                <div class="vsv-overlay"></div>
                <div class="vsv-bloc-wrapper">
                  <div class="vsv-titles">
                    <h2 class="vsv-title">Çanta</h2>
                    <div class="vsv-links">
                      <a
                        href="/tr/kadin/one-cikanlar/ayın-trendleri_d21332319"
                        class="vsv-link"
                        title="Go to Ayın trendleri"
                        ><span class="hvr-underline-fix2"
                          >Daha fazla keşfet</span
                        ></a
                      >
                    </div>
                  </div>
                </div>
              </div>
            </div>
            <div class="vsv-blocs">
              <div class="vsv-bloc" style="background-color: #14131166">
                <div
                  class="vsv-image data-vsvbg-50"
                  data-device="desktop,tabletlandscape,tablet"
                  data-akaset=""
                  data-vsvbg="//staticpages.mngbcn.com/homes/images/fw22/she/noviembre/she_landing_keytrends_0711.jpg"
                  data-vsv-bg-processed="//staticpages.mngbcn.com/homes/images/fw22/she/noviembre/she_landing_keytrends_0711.jpg?imwidth=429&amp;imdensity=1&amp;impolicy=set_13"
                  style="
                    background-image: url('//staticpages.mngbcn.com/homes/images/fw22/she/noviembre/she_landing_keytrends_0711.jpg?imwidth=429&amp;imdensity=1&amp;impolicy=set_13');
                    transition: opacity 0.5s ease 0s;
                    opacity: 1;
                  "
                ></div>

                <div class="vsv-overlay"></div>
                <div class="vsv-bloc-wrapper">
                  <div class="vsv-titles">
                    <h2 class="vsv-title">Ayın trendleri</h2>
                    <div class="vsv-links">
                      <a
                        href="/tr/kadin/one-cikanlar/ayın-trendleri_d21332319"
                        class="vsv-link"
                        title="Go to Ayın trendleri"
                        ><span class="hvr-underline-fix2"
                          >Daha fazla keşfet</span
                        ></a
                      >
                    </div>
                  </div>
                </div>
              </div>
            </div>
          </div>
        </div>
      </div>
      <div class="vsv-bannercontainer" data-bannerid="9542">
        <div
          class="vsv-box"
          id="vsv-abrigos-she"
          data-id="9542"
          data-islinkable="/tr/kadin/palto_c67886633"
        ></div>
      </div>
    </div>
  </section>
</template>
